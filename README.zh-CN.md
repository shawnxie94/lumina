...
- **AI 解读流水线**：支持摘要、总结、金句、大纲、翻译、自动分类、自动标签与相关文章推荐，后台任务可监控、取消、重试。
- **评论与协作能力**：支持文章评论与回复、管理员审核/隐藏/删除、GitHub/Google OAuth 登录，以及评论敏感词过滤。
- **后台控制中心**：集中管理站点基础信息、首页文案、模型 API（通用/向量）、提示词、推荐策略、分类、评论与存储配置。
- **运维与可观测能力**：提供 AI 任务时间线、调用计量（次数/Tokens/费用）、接口错误/任务失败通知中心，以及关键后端健康状态。
- **内容生命周期管理**：支持本地媒体存储/压缩/清理、Markdown 导出、公开 RSS 订阅，以及 JSON 备份导出与严格增量导入恢复。
...
- **RSS 订阅**：支持公开 RSS 订阅链接，并可按分类、标签过滤输出。

- **通知中心**：在页面头部统一查看 AI 任务失败与接口错误通知。
