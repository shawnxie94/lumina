...
const twitterAdapter: SiteAdapter = {
  ...
  getContentSelector: () => {
    ...
    const hasArticleContent = document.querySelector('main h2, main [role="heading"][aria-level="2"]');
  ...
  getAuthor: () => {
    ...
    if (isArticleMode) {
      const authorLink = document.querySelector('main a[href^="/"][href$="' + window.location.pathname.split('/')[1] + '"]');
      if (authorLink) {
    ...
    if (pathMatch) {
      ...
      if (userLink) {
        ...
        for (const span of spans) {
          ...
          if (text && !text.startsWith('@') && !text.includes('·') && text.length > 0 && text.length < 50) {
            return text;
          }
        }
      }
  ...
  getTitle: () => {
    ...
    if (isArticleMode) {
      ...
      if (mainContent) {
        ...
        for (let i = 0; i < Math.min(children.length, 5); i++) {
          ...
          if (text && text.length > 10 && text.length < 200 && !text.includes('@') && !el.querySelector('img')) {
            return text;
          }
        }
      }
    }
    const h2 = document.querySelector('main h2, main [role="heading"][aria-level="2"]');
    if (h2) {
    ...
    if (metaTitle) {
      ...
      return content.replace(/\s+on\s+(?:X|Twitter).*$/i, '').replace(/\s*[/／]\s*X$/, '').trim();
    }
  ...
  preProcess: () => {
    ...
    document.querySelectorAll('main article').forEach((quotedTweet) => {
      ...
      const textEl = quotedTweet.querySelector('div[lang], [data-testid="tweetText"]');
