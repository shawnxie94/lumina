...
class SettingsController {
	...
	setupSidebarNavigation() {
		try {
			...
			if (hashSectionId && document.getElementById(hashSectionId)) {
				this.switchSection(hashSectionId);
			} else {
				this.switchSection("section-api");
			}
		} catch (error) {
	...
	setupEventListeners() {
		document
			...
			?.addEventListener("click", () => this.saveApiConfig());
		document
		...
		document
			.getElementById("resetKeywordsBtn")
			?.addEventListener("click", () => this.resetKeywords());

		document
			.getElementById("apiHostInput")
			?.addEventListener("change", () => this.checkApiHealth());

		document
			.getElementById("refreshLogsBtn")
			?.addEventListener("click", () => this.loadErrorLogs());
		document
	...
	async renderCategoryKeywords() {
		...
		for (const category of this.#categories) {
			const keywords =
				customKeywords[category.name] ||
				getDefaultKeywordsForCategory(category.name);
			const keywordsText = keywords.join(", ");
			const isCustom =
				customKeywords[category.name] &&
				customKeywords[category.name].length > 0;
      ...
      `;
			...
			textarea?.addEventListener("input", () =>
				this.updateKeywordCount(itemEl, textarea),
			);
